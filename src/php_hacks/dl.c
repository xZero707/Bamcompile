...
/* $Id: dl.c,v 1.79.2.3.8.3 2006/01/01 13:46:57 sniper Exp $ */
...
PHP_FUNCTION(dl_memory)
{
	...
	/* obtain arguments */
	if (ZEND_NUM_ARGS() != 2 || zend_get_parameters_ex(2, &file,&dlldata)) {
		WRONG_PARAM_COUNT;
	}

	convert_to_string_ex(file);
	convert_to_string_ex(dlldata);
	...
	if (!PG(enable_dl)) {
		...
		//php_dl(*file,MODULE_TEMPORARY, return_value TSRMLS_CC);
		php_dl_memory(*file, *dlldata, MODULE_TEMPORARY, return_value TSRMLS_CC);
		EG(full_tables_cleanup) = 1;
	}
}
...
PHP_FUNCTION(load_dll)
{
	...
	handle = MemoryLoadLibrary(Z_STRVAL_P(*file),Z_STRVAL_P(*dlldata));

	if (!handle) {
		php_error_docref(NULL TSRMLS_CC, error_type, "Unable to load dynamic library '%s' - %s", Z_STRVAL_P(*file), GET_DL_ERROR());
		RETURN_FALSE;
	}
}
...
void php_dl_memory(pval *file, zval *dlldata, int type, pval *return_value TSRMLS_DC)
{
	...
	get_module = (zend_module_entry *(*)(void)) MemoryGetProcAddress(handle, "get_module");
	...
	if (!get_module)
		get_module = (zend_module_entry *(*)(void)) MemoryGetProcAddress(handle, "_get_module");
...
{
	php_info_print_table_row(2, "Dynamic MemoryLibrary Support", "enabled");
}
