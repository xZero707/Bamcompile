...
function get_all_files($dir,$subpath='')
{
	...
	while (false !== ($entry = $d->read()))
	{
		if(!($entry=='.') & !($entry=='..'))
		{
			if(is_dir($dir.'/'.$entry))get_all_files($dir.'/'.$entry,$subpath.$entry.'/');
			else $includefiles[] = array($sourcepath.$subpath.$entry,$destpath.$subpath.$entry);
		}
	}
}
...
while(list($nr,$val)=each($argv))
{
	...
	{
		$val = strtolower($val);
		if($val=='-w')$option_windowed = true;
		else if($val=='-c')$option_compress = true;
		else if($val=='-d')$option_noencode = true;
		else if(strpos($val,'.php')>-1)$mainfile = $val;
		else if(strpos($val,'.exe')>-1)$outfile = $val;
		else if(strpos($val,'.bcp')>-1)$projectfile = $val;
		else if(substr($val,0,3)=='-e:')$extensions[] = substr($val,3);
		else if(substr($val,0,3)=='-i:')$iconfile = substr($val,3);
		else if(is_dir($val))$projectdir = $val;
	}
}
	...
	while(list(,$row)=each($projectdata))
	{
		...
		if(substr($row,0,5)=='embed')
		{
			...
			if(is_dir($embedpath))
			{
				...
				if(substr($sourcepath,strlen($sourcepath)-1,1)!= '\\')$sourcepath.='\\';
		...
		if(substr($row,0,11)=='destination')
		{
			...
			if(substr($destpath,strlen($destpath)-1,1)!= '/')$destpath.='/';
...
if($mainfile & $projectfile)
{
	...
	{
		if(basename($file[0])==$mainfile)$found=true;
	}
	if(!$found)
	{
...
if($option_windowed)print "Windowed application\n";
if($option_compress)print "Compress\n";
if($option_noencode)print "Do not encode\n";
if($mainfile)print "Mainfile: $mainfile\n";
if($outfile)print "Outfile: $outfile\n";
if($iconfile)print "Icon: $iconfile\n";
if($projectdir)print "Project dir: $projectdir\n";
...
while(list($nr,$file)=each($includefiles))
{
	...
	{
		if(basename($file[0])==basename($ext))$includefiles[$nr]=array("","");
	}
}
reset($includefiles);
reset($extensions);
...
if($iconfile)
{
	...
	while(list($nr,$file)=each($includefiles))
	{
		if(basename($file[0])==basename($iconfile))$includefiles[$nr]=array("","");
	}
	reset($includefiles);
}
...
if(count($extensions)>0)
{
	...
	{
		$extension_file = $file;
		if(!file_exists($extension_file))$extension_file=$path.$file;
...
if(count($extensions)>0)res_set($outfile,"PHP","EXTENSIONS",implode(';',$extension_loadlist));
